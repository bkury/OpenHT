...
if sys.argv[1].startswith('Location'):
    ...
    if (not location):
	log('Trying GeoIP lookup')

	try:
		request = requests.get(GCURL, timeout=10)
		data = request.json()
	except:
		log('GeoIP lookup failed')
		ADDON.setSetting('Location1','Unknown')
	...
	if data:
		...
		if city:
			...
			data = get_data(LCURL, search)

			for item in data:
				city = item['qualifiedName']
				cityid = str(item['woeid'])
				break

			if cityid:
				ADDON.setSetting('Location1',city)
				ADDON.setSetting('Location1id',cityid)
				log('geoip location: %s' % city)
				log('geoip locationid: %s' % cityid)
			else:
				ADDON.setSetting('Location1','Unknown')
